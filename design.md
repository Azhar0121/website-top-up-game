...
## Panduan Penggunaan Desain (Do's and Don'ts)

### Yang Harus Dilakukan (Do)
- Selalu gunakan variabel CSS yang telah ditentukan (`var(--color-*)` untuk customer, `var(--admin-*)` untuk admin) agar konsistensi warna tetap terjaga.
- Gunakan font **Baloo 2** khusus untuk elemen judul/heading dan **Plus Jakarta Sans** untuk seluruh teks isi dan form.
- Pastikan semua elemen tombol aksi dan filter chip menggunakan bentuk kapsul melengkung murni (`rounded-pill` / 999px).
- Pastikan state `:hover` pada seluruh elemen interaktif di halaman customer memiliki kontras yang cukup terhadap latar belakang gelap.
- Gunakan efek *skeleton loading* saat memuat katalog produk atau data game async.

### Yang Tidak Boleh Dilakukan (Don't)
- Jangan menambahkan warna latar belakang cerah polos (seperti putih murni) pada halaman publik customer yang merusak tema gelap.
- Jangan menggunakan font default sistem seperti Times New Roman atau Arial jika font Google Fonts berhasil dimuat.
- Jangan menghapus garis tepi (*border*) pada kartu produk atau form input di antarmuka publik.
- Jangan mempergunakan warna aksen Pink Neon atau Kuning Amber secara berlebihan di luar fungsi penekanan dan interaksi visual utama.
- Jangan menggunakan efek gradasi solid mencolok pada kartu admin yang mengganggu keterbacaan data tabel.
